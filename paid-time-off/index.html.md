...
Paid time off is in addition to any [official public holidays](#holidays) that apply to the country where you live, and [Friends and Family days](/company/family-and-friends-day/). When accounting for in [PTO by Deel](#pto-by-deel), please use the label `Public Holiday` or `Friends and Family`. This helps us to ensure that all of our Team Members are meeting at least the [Statutory Vacation Requirements](#statutory-vacation-requirements).
...
1. If during a PTO event, you realize that certain days will need to be changed from PTO to Sick Time, or another category, please email `leaves@domain` with your specific request and the Absence Management Team will complete your request, per the [instructions](/handbook/paid-time-off/#instructions-for-people-ops-and-total-rewards-to-update-pto-by-deel-events). Events can only be changed after the PTO by Deel event has completed itself.
...
1. You should add the time off using [PTO by Deel](#pto-by-deel) to Slack. Please use the following syntax which uses the [ISO 8601](https://en.wikipedia.org/wiki/ISO_8601) date notation in order to avoid confusion: `OOO from YYYY-MM-DD to YYYY-MM-DD, please contact XXX for assistance`.
    ...
    * When your time off starts, PTO by Deel will automatically update your Slack status to communicate your time off. This will prevent you from appearing in [reviewer roulette](https://docs.gitlab.com/ee/development/code_review.html#reviewer-roulette).
