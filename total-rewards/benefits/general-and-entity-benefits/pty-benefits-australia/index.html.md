...
Time in lieu should be requested via [PTO by Deel](/handbook/paid-time-off/#pto-by-deel) by selecting the `On-Call Time in Lieu` option.
...
2. Select `Out Sick` in PTO by Deel to ensure that leave entitlements are properly tracked.      
...
* This leave runs concurrently with GitLab PTO.  Team members should select `Vacation` in PTO by Deel to ensure that leave entitlements are properly tracked.
...
This leave runs concurrently with GitLab PTO. Team members should select `Bereavement Leave` in PTO by Deel to ensure that leave entitlements are properly tracked.
...
* Team members must select `Public Holiday` in PTO by Deel to ensure that statutory entitlements are properly tracked.
...
* This leave runs concurrently with GitLab PTO. Team members must select `Mandatory Civilian Service` in PTO by Deel to ensure that statutory entitlements are properly tracked.
...
* Designate any time off for illness as `Out Sick` in PTO by Deel to ensure that sick leave entitlement is properly tracked.
...
* Team members must designate time off for injury as `Out Sick` or `Vacation` in PTO by Deel if they wish to use sick leave or annual leave for the first week of an injury. 
...
* This leave will run concurrently with GitLab PTO. 
    - If you do not want to select `Out Sick` in PTO by Deel, you may email `leaves@gitlab.com` with the date(s) that you will be taking special leave. 
...
* Team members must select `Parental Leave` in PTO by Deel to ensure that their time off is properly tracked.
...
* To designate and track Stress Leave properly, select as `Out Sick` in PTO by Deel.
...
* Team members must select `Mandatory Civilian Service` in PTO by Deel to ensure that statutory entitlements are properly tracked.
...
* This leave runs concurrently with GitLab PTO. Team members must select `Public Service/Volunteer` in PTO by Deel to ensure that statutory entitlements are properly tracked.
...
* Jury Service Leave runs concurrently with GitLab PTO. Team members must select `Jury Duty` in PTO by Deel to ensure that statutory entitlements are properly tracked.
...
* This leave runs concurrently with GitLab PTO. Team members must designate this time off as `Bereavement Leave` in PTO by Deel to ensure that statutory entitlements are properly tracked.
...
* Team members must designate this time off as `Out Sick` in PTO by Deel to ensure that statutory entitlements are properly tracked.
...
* This leave runs concurrently with GitLab PTO. Team members must designate this time off as `Out Sick` in PTO by Deel to ensure that statutory entitlements are properly tracked.
...
* Team members must designate this time off as `Vacation` in PTO by Deel to ensure that statutory entitlements are properly tracked.
...
* Team members must designate this time off as `Out Sick` in PTO by Deel. 
...
* This leave runs concurrently with GitLab PTO. Team members must designate this time off as `Vacation` in PTO by Deel.
