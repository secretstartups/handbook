...
1. Verify that the **new** subscription is for **a plan with seats**. If it is for compute credits or other consumption, STOP and review the CI purchases to ensure we don't overwrite and lose their existing compute credits (this can be done by an L&R expert).
