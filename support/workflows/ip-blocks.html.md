...
Note that CloudFlare utilizes [MaxMind](https://www.maxmind.com/en/geoip2-precision-demo) for GeoLocation blocks as required by US Export Regulations. These blocks are separate and cannot be reversed. You can enter an IP address here to determine how an IP address is classified and verify against current US embargoed countries. A user might considering [requesting a data correction](https://support.maxmind.com/geoip-data-correction-request/) of their IP address but it is not guaranteed and GitLab has no control over this process.
