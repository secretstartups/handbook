...
| [Support Engineer Escalations](https://gitlab.com/gitlab-com/support/internal-requests/-/issues) | Subscriptions App issue which may be billing or bad data related    |
| [Subscriptions App](https://gitlab.com/gitlab-org/customers-gitlab-com/issues) | Issue is caused specifically by something within the Subscriptions App    |
...
For GitLab.com, there is no artefact to be uploaded but the subscription (seen in the Subscriptions App) has to be linked to their namespace (on GitLab.com). If the portal and Gitlab.com don't have the link, then, GitLab.com won't know that a subscription exists.
...
1. **User reports an inability to upgrade from one paid plan to another.**
    ...
    - GitLab.com users can upgrade their paid plan to a higher tier in the [Subscriptions App](https://customers.gitlab.com/customers/sign_in) by selecting the `Upgrade` button beneath the subscription widget.
...
1. **User wants to trial a plan other than Gold on GitLab.com.** GitLab.com [only offers the self-service ability to trial the Gold subscription](https://gitlab.com/gitlab-org/customers-gitlab-com/issues/409). With manager approval, GitLab.com support can assist with trials of other plans. Ask the user to create the GitLab.com Gold trial, then change the plan via the Subscriptions App admin `GitLab Groups` page.

1. **User receives an error during the purchasing process within the [Subscriptions App](https://customers.gitlab.com/customers/sign_in).** When purchasing a subscription via the Subscriptions App if a user receives an error about an address or credit card, check the address listed in the `My Account` section of the Subscriptions App. This is typically due to a bad address (city/state/zip code). If the address is correct and the error persists, check sentry for a relevant error and file an issue in the [Subscriptions App tracker](https://gitlab.com/gitlab-org/customers-gitlab-com/-/issues/) as necessary.
...
1. **User doesn't know the steps to purchase a GitLab.com subscription.**
   ...
   - Create an account in the [Subscriptions App](https://customers.gitlab.com/customers/sign_up)
   - Associate GitLab.com account with Subscriptions App account. First log into GitLab.com, then in another tab in the same browser open the Subscriptions App and navigate to `My Account` and select `Change Linked Account`
...
1. **User doesn't see their group during purchase process.** If the customer can't see their Group when purchasing a subscription, one of the below options are likely happening:
   ...
   - They need to associate their GitLab.com account with their Subscriptions App account
...
1. **User purchased CI minutes but they aren't associated with their group.**
    ...
    1. Reset the pipeline minutes via GitLab.com admin UI. Sometimes this shakes things loose and the association actually works.
        - If this works, stop and notify user.
        - If this is unsuccessful, create a console esclation issue ([example](https://gitlab.com/gitlab-com/support/internal-requests/issues/1050)). Include Zendesk link, Subscriptions App link, Customer ID, Group Name, Group ID, Subscription (number/name). Assign the `Customers Console escalation` label.
...
1. **User wants to know if they should add more users midterm or during the renewal process.** - Using the "Add more seats" button (for self-managed) in the Subscriptions App at any time during their subscription will charge a pro-rated amount for the additional users. If they wait to purchase the additional users at the renewal via the [true-up users](https://about.gitlab.com/pricing/licensing-faq/#what-does-users-over-license-mean) process, the users will be charged for 12 months of use.

1. **User reports they cannot log into the [Subscriptions App](https://customers.gitlab.com/customers/sign_in).** - The user is likely logging into the wrong system (usually GitLab.com). Provide the user with the proper url as demonstrated in this example ticket[#144710](https://gitlab.zendesk.com/agent/tickets/144710).
...
1. **User wants to know when they will receive the license key or who the license will be sent to.** The license key will be emailed, [to the email address associated with the subscription in the Subscriptions App](https://gitlab.com/gitlab-org/customers-gitlab-com/-/issues/1044#note_318221832), once subscription payment is approved internally by the billing team.  If there is an unusually long delay, assign the ticket to the "License, Renewals and Upgrades" queue or [open an issue](https://gitlab.com/gitlab-com/support/internal-requests/issues/new?issuable_template=license%20issue) using the "license issue" template.
...
1. Copy of invoice - _check first if the invoice is available in the [Subscriptions App](https://customers.gitlab.com/customers/sign_in),  if  yes, walk the customer through locating the invoices under Payment History for future self-service ability._
...
1. Subscription #
1. Subscription information - copy & paste from `Manage Purchases` in [Subscriptions App](https://customers.gitlab.com/customers/sign_in)
1. Zuora ID - available in the [Subscriptions App](https://customers.gitlab.com/customers/sign_in)  under the  `Edit` tab
1. Salesforce Account ID - available in the [Subscriptions App](https://customers.gitlab.com/customers/sign_in)  under the  `Edit` tab
...
If the user sees a subscription in the Subscriptions App but doesn't see the paid plan active in their GitLab.com account, have the user associate the group with the subscription.

Associating a group with a subscription in the Subscriptions App:
...
If the user doesn't see a subscription in their Subscriptions App:
...
1. Log into the [Subscriptions App](https://customers.gitlab.com/customers/sign_in) using the generic
email. Password is stored in 1Password.
1. Locate the proper account in the Subscriptions App and navigate to the `Edit` page
...
## Associating Subscriptions App subscriptions with additional accounts

Sometimes customers ask that a colleague be given the ability to manage a subscription. This can be accomplished provided they also have a Subscriptions App account.

Prior to following the below steps, ensure the requester should have access to the subscription.
1. For self-managed, send the [Change Subscriptions App Contact](https://gitlab.zendesk.com/agent/admin/macros/360028045239) Zendesk macro. Be sure to copy the existing Subscriptions App contact on the reply.
...
3. [Subscriptions App Admin Docs](/handbook/internal-docs/customers-admin/) _(internal audience)_
