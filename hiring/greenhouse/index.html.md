...
In an effort to eliminate bias and prevent unnecessary editing of scorecards, Interviewers - unlike Hiring Managers - are **not** able to see scorecards submitted by fellow Hiring Team Members. If and when there are scorecard disparities (e.g. a `Strong Yes` provided by one Interviewer and a `Strong No` provided by another), it will be up to the requisition's Hiring Manager and Recruiter to discuss that disparity and determine next steps. If an Interviewer has a concern or feels that a topic needs further probing, they can leave a note for others via the `Note for Other Interviewer` section beneath the `Key Take-Aways` textbox in the scorecard. When leaving a note for other Interviewers, please be sure to check the `Visible to Interviewers` box.
