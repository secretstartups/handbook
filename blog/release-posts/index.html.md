...
Each month the release post manager may need help with technical hurdles during the release post process. In order to provide the the release post, which is a time sensitive and highly visible asset fore customers and users, with adequate technical advisement and support, we have a partnership with the GitLab development team to leverage Dev Escalation process (insert exact name/link) via the Slack XX channel. This insures that at all times, if something breaks that the release post team can not resolve themselves, they have access to technical experts for resolution.
