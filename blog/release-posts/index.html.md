...
Product Managers can volunteer for any release that doesn't have someone assigned yet. To add yourself as release post manager or release post manager's shadow, simply submit an MR to update the `/data/release_post_managers.yml` file which will display your name on the [Release Post Scheduling page](managers/). Otherwise, PMs will be assigned using a fair scheduling principle:
