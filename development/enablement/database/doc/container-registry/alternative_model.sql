...
CREATE TABLE public.media_types (
    ...
    type text not null,
    CONSTRAINT ck_media_types_type_length CHECK ((char_length(type) <= 255)),
    CONSTRAINT pk_media_types PRIMARY KEY (id),
    CONSTRAINT uq_media_types_type UNIQUE (type)
);

-- Operations that acquire a row FOR UPDATE lock:
-- 1. creating another layer referencing this blob (in layers)
-- 1. deleting a blob record (from storage and database)
-- => Those operations have to get serialized => TODO: detail this more
...
CREATE TABLE public.manifests (
    ...
    CONSTRAINT uq_manifests_digest_algorithm_digest_hex UNIQUE (repository_id, digest)
...
CREATE TABLE public.manifest_references (
    ...
    -- TODO: CONSTRAINT ck_manifest_references_parent_id_child_id_differ CHECK (parent_id <> child_id),
    FOREIGN KEY (repository_id, parent_id) REFERENCES manifests (repository_id, id),
    FOREIGN KEY (repository_id, child_id) REFERENCES manifests (repository_id, id)
) PARTITION BY HASH (repository_id);
...
CREATE TABLE public.layers (
     ...
     created_at timestamp WITH time zone NOT NULL DEFAULT now(),
     CONSTRAINT pk_manifest_layers PRIMARY KEY (repository_id, id),
     CONSTRAINT uq_manifest_layers_manifest_id_digest UNIQUE (repository_id, manifest_id, digest),
     FOREIGN KEY (repository_id, manifest_id) REFERENCES manifests (repository_id, id) ON DELETE CASCADE
) PARTITION BY HASH (repository_id);
...
CREATE TABLE public.blobs_layers (
  ...
  -- instead of tracking references, this could be a 1:1 with blob and counting how many references exist
  -- downside: serializes updates and consistency might become an issue 
  repository_id bigint NOT NULL,
  layer_id bigint NOT NULL,

  CONSTRAINT pk_blobs_layers PRIMARY KEY (digest, id),
  CONSTRAINT uq_blobs_layers UNIQUE (digest, layer_id),
  FOREIGN KEY (repository_id, layer_id) REFERENCES layers (repository_id, id) ON DELETE CASCADE
) PARTITION BY HASH (digest);

-- TODO: move tag into manifests
CREATE TABLE public.tags (
     ...
     CONSTRAINT uq_tags_name_repository_id UNIQUE (repository_id, name),
     CONSTRAINT ck_tags_name_length CHECK ((char_length(name) <= 255)),
     FOREIGN KEY (repository_id, manifest_id) REFERENCES manifests (repository_id, id) ON DELETE CASCADE
) PARTITION BY HASH (repository_id);

-- Not entirely sure here - if we want this, we might have to add another reverse-lookup table digest -> repository_id
CREATE TABLE public.repository_blobs (
      ...
      CONSTRAINT uq_repository_blobs_repository_id_blob_id UNIQUE (repository_id, blob_digest)
) PARTITION BY HASH (repository_id);
...
INSERT INTO media_types (type) VALUES ('default');
