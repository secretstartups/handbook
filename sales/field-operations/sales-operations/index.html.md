...
5. Since many opportunities and accounts are missing the Datafox country fields, "ZoomInfo Country" and "Billing Country" fields are used also to populate as well.
...
7. If there is no Datafox, ZI, or billing information on the account or opportunity of the company, look up the company by name or by contact and or the account's billing information and perform a google search to populate as much information as possible. If no information is found, tag billing ops to obtain the zuora billing address information to populate the billing fields on the account. 
...
3. [Sales Admin Opps Dashboard](https://gitlab.my.salesforce.com/01Z4M000000oYRM)
...
### DocuSign
1. SalesOps supports provisioing for DocuSign from within SFDC.  For the DocuSign Portal/ standalone appication, please refer to the tech stack for the provisioner. 
...
1. NOTE: Sales Ops (Provisioners from witin SFDC) need to be added as "Associated User" in Docusign Portal and Authorize themselves by clicking the activation link sent to their email. Please refer to the tech stack for the DocuSign Portal provisioner for troubleshooting this step if needed.
