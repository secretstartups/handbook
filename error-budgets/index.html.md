...
- _**Completed**_ The SLO targets currently used for Error Budgets are coupled with the alerting used for Infrastructure monitoring. We will separate these two so that we have one SLO target for alerting, and a separate SLO target for Error Budgets. The capability to accomplish this differentiated setting is detailed in [issue](https://gitlab.com/gitlab-com/gl-infra/scalability/-/issues/1239), however it has been decided that will will not utilize different SLO targets at this time. Instead, we will move to a longer apdex request duration threshold (see next item)
- _**Completed**_ SLI calculations use a shared (single) request duration threshold which is not appropriate for all endpoints. The threshold is also a value shared with alerting. The SLI request duration threshold used for error budget calculation will be decoupled from the value for alerting and set higher. This will reduce our expectations for latency used for Error Budgets while keeping the expectations high for the purposes of alerting. This change has been completed and the impact will take effect incrementally through the first 28 days.
