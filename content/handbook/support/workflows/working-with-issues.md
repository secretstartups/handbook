...
- It's often worth searching across the whole `gitlab.org` group, particularly if you're not sure which project issues might have been raised in.
  - If you can filter the search results, use [issue search](https://gitlab.com/groups/gitlab-org/-/issues).
  - Advanced search has the advantage of also searching Epics, and if you're searching for an error or for other text generated by the product, this will also search all our code repositories as well.
- Searching in the issue view is usually more efficient if you can filter the results:
  - If you are looking for a change that was implemented in a particular GitLab release, filter on Milestone.
    - Consider also searching merge requests by milestone as well.
  - If you know which product area is responsible for a feature, filter on the `~group::` label. The [features by group page](../../product/categories/features) is good for finding out the correct group.
...
- Consider using an reaction on all issues that you come across, and any you raise. For example `bookmark`, `reminder ribbon` or pick something really unusual! You can then filter on `My-Reaction` and search within those results.
