...
1. Option for unactivated licenses sent to a contact that has typos in an email address. Occasionally, Support receives requests to correct the typo in the customer's contact email and resend the license to a valid email address. Support can [check the Mailgun logs](/handbook/support/license-and-renewals/workflows/customersdot/troubleshoot_errors_while_making_purchases/#troubleshooting-email-delivery-from-customerdot) for the evidence that the license email was sent, yet never delivered to the customer. As the email with the typo doesn't exist, the Mailgun log entry will display a failed mail delivery attempt (commonly shown as a **550 error code**). When you have the evidence of a failed delivery attempt, you can proceed with fixing the typo in the email address. As the requests to fix an email typo are raised in a form of [IRs (Internal Requests)](/handbook/support/license-and-renewals/workflows/working_internal_requests/), confirm the customer's valid email address with the Sales representative who opened a request, and advise them to fix the typo (if present) in the SFDC records.
