...
1. For SaaS customer you can verify if the ticket requestor is an owner in the namespace attached to the subscription by asking the individual to:
   - Login to their gitlab.com namespace as a group owner.
   - Open an API call to their namespace URL: "https://gitlab.com/api/v4/namespaces/<customer_namespace>/gitlab_subscription" and then copy/paste the returned data to the support ticket.
   - Using a gitlab.com admin account you can then verify the accuracy of the subscription data provided and approves or deny the verification request based upon this.
