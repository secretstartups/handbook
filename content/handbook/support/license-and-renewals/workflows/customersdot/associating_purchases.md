...
1. Option for unactivated licenses sent to a contact that has typos in an email address. Occasionally, Support receives requests to correct a typo in a customer's contact email address and to resend the license to a valid email address. Support can [check the Mailgun logs](/handbook/support/license-and-renewals/workflows/customersdot/troubleshoot_errors_while_making_purchases/#troubleshooting-email-delivery-from-customerdot) to verify that the license email was sent but failed to reach the intended customer. Given the email with the typo does not exist, the Mailgun log entry will display a failed mail delivery attempt (commonly shown as a **550 error code**). Upon confirming a failed delivery attempt, the next step requires you to rectify the email typo. As requests to fix an email typo are raised via an [Internal Request Form](/handbook/support/license-and-renewals/workflows/working_internal_requests/), you should validate the customer's accurate valid email address with the Sales representative who submitted the request, and also advise them to fix the typo (if present) in the associated SFDC records.
