...
1. `Context` section includes: The goal(s) of the meeting; clear pre-work or pre-watch/read instructions; intent and expected outcomes using [low-context communication]({{< ref "./effective-communication" >}}).
...
Many meetings can be avoided by understanding how to [work well asynchronously]({{< ref "./asynchronous" >}}).

For a deeper dive on how GitLab implements meetings in a remote work environment, visit our [all-remote meetings guide]({{< ref "./meetings" >}}). To learn more about how GitLab communications, visit our [Communications page]({{< ref "communication" >}}).

## Q: What is a live doc meeting?

A live doc meeting is a GitLab specialty. It's a work-related meeting (e.g. not a [coffee chat]({{< ref "./informal-communication#coffee-chats" >}})) where you document everything live (yes, everything)! Start documenting before the meetings begins by attaching a shared, editable agenda document to the calendar invite.
...
There are two ways to participate in a live doc meeting: either synchronously, like when [joining a live Zoom call]({{< ref "communication#video-calls" >}}), or [asynchronously]({{< ref "./asynchronous" >}}), by reviewing the documented discussion along with any recorded video after the meeting is over.

## Q: When are live doc meetings held?

Live doc meetings are usually held in the host's time zone. However, leaders should [rotate meetings to accommodate a more diverse array of time zones]({{< ref "./asynchronous#remove-time-zone-bias" >}}) as well as record them so that others can watch at a later time.
