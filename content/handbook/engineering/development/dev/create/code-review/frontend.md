...
{{< stable-counterparts manager-role="Engineering(.*)Manager(.*)Create:Code Review" role="[,&] \(Create(.*)|Create:Code Review|Create \(Code Review|Director(.*)Create|Principal(.*)Create|Group(.*)Create" >}}
