...
The scope of the StORM program is limited to operational, technology-agnostic security risks, e.g., inadequate physical security controls. These risks can be identified in many ways including Risk Assessments, reports from team members, or as a result of compliance activities. There may be instances where an applications role is so significant to internal security controls that we may create risks specifically for that system. This should largely be limited to GitLab.com as it's use is pervasive in all that we do.
