...
- **Hash Functions** - The SHA-2 family of hash algorithm, with SHA-256 as the minimum. Ideally SHA-384 or higher would be better, although there might be less support particularly in third party software.
  ...
  - [SLH-DSA](https://nvlpubs.nist.gov/nistpubs/FIPS/NIST.FIPS.205.pdf) is a NIST-approved quantum safe algorithm. SLH-DSA strength lies in functioning as a replacement of stateful hash algorithms such as LMS and XMSS (SLH-DSA is stateless). Just note that SLH-DSA can only be used in a FIPS environment if ti is using a FIPS-certified cryptographic module. For more information see [this section](#quantum-cryptography). 
