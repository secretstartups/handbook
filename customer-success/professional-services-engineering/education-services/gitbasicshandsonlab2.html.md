...
1. You will be prompted to select the location in which your key will be saved. Press **Enter** to leave it at the default.
1. You may be prompted to create a passphrase. Leave it blank or enter a passphrase of your choice, and then hit **Enter**. If you do use a passphrase, you'll have to enter it every time you push or pull code from GitLab.com.

### Add an SSH Key to Your GitLab profile
1. In the top right-hand corner of the screen, locate your user avatar and click the **down arrow**.
1. From the dropdown menu, click the **Edit profile** item.
1. On the left-hand side of the screen, click on **SSH Keys**.
1. If you have no SSH keys added to your profile, you will land on the **Add an SSH Key** screen.
1. Return to your terminal/command session. To retrieve the SSH key you created, move into the folder you saved it to by typing `cd {file path}`
1. Type `ls -al` to see two key files: a public key and a private key. *The public key is called* `id_rsa.pub` *and is what you need to share with GitLab.*
1. Type `cat id_rsa.pub` to print the contents of your public key. Copy those contents to your clipboard.
...
1. Your local computer is now authenticated to push and pull (interact) with GitLab.

### Copy (clone) a project repo to your local machine
1. Use the top navigation bar to get back to your **Project Overview** by clicking **Projects > Your projects > Top Level Project**.
1. Locate the blue **Clone** button and click the dropdown arrow.
...
1. Type `mkdir training` to create a new directory.
1. Type `cd training` to move into your new directory.
1. Type `git clone <URL you copied previously>` to copy the git repository from your **Top Level Project** onto your local machine.
1. Move into the repository you just cloned by typing `cd top-level-project`. This is where git will track your changes, and where you will interact with the repo and git.
1. Type `ls -al` to see a list of all files in your current directory, including hidden files.
1. Type `git status`. You will see a message saying your working tree is clean.

### Work on a branch
1. Type `git checkout -b temporary_branch`
1. Type `git branch -a` to see all branches.
...
1. Using the editor of your choice, add a new line `a second line added to master branch locally` at the end of `README.md`. Save the file.
...
1. You have now created a record or point-in-time snapshot of the file that you can refer back to if needed.
...
1. In your terminal/command window, type `git add README.md`
1. Type `git commit -m "modified README.md"`
1. Type `git log`
1. Type `git push`
...
1. Navigate to your GitLab.com project in your browser and confirm that your local changes were pushed up.

### Simulate a change on the remote **temporary_branch**
In this section, let's simulate someone else in your organization making a change to the remote copy of **temporary_branch** that lives on GitLab.com. When we're done with this section, the remote and local versions of **temporary_branch** will be different: the code on that branch will have moved under your feet (so to speak)! In the section that follows this one, we'll see how to reconcile this difference.
...
1. You are now looking at files in **temporary_branch**. Click on **README.md**.
1. Click the **Web IDE** button.
...
1. Click the **Commit** button to finalize the changes on the remote copy of **temporary_branch**. Since you made this change on the server, the server repository is now one commit ahead of your local repository.
...
The `git fetch` command retrieves the updated state of remote branches without updating the contents of your local branches. In other words, this tells you how many commits your local branches are behind the remote branches, but it won't make any changes to your local branches.

1. Return to your terminal/command window and type `git fetch`. If you are prompted for your SSH passphrase, enter it.
1. Type `git status` and review the updated status of your local branch.

### Pull from the remote (upstream) repository
You now need to explicitly tell git to update the contents of your local **temporary_branch** by merging in changes from the remote **temporary_branch**.
...
1. Switch to your **master** branch by typing `git checkout master`
1. Type `git merge temporary_branch` to incorporate all changes from your local **temporary_branch** (in this case, just the modified `README.md`) into your local **master** branch.

### Update the remote repository
1. In your terminal/command window, type `git status` to see that there are no changed files that you need to stage or commit and to confirm that you are on **master** branch.
1. Type `git push` to update the remote copy of **master** branch with any changes from your local copy.
1. Navigate back to GitLab.com and view `README.md` in your project's **master** branch to view the changes.
