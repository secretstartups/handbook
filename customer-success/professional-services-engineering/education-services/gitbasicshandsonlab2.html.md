...
2. You will be prompted to select the location in which your key will be saved. Press **Enter** to leave it at the default.
3. You may be prompted to create a passphrase. Leave it blank or enter a passphrase of your choice, and then hit **Enter**. If you do use a passphrase, you'll have to enter it every time you push or pull code from GitLab.com.
...
6. Type `ls -al` to see two key files: a public key and a private key. *The public key is called* `id_rsa.pub` *and is what you need to share with GitLab.*
7. Type `cat id_rsa.pub` to print the contents of your public key. Copy those contents to your clipboard.
...
2. You have now created a record or point-in-time snapshot of the file that you can refer back to if needed.
3. Type `git status` to see that the staging area is empty again, following the commit.
...
2. In your terminal/command window, type `git add README.md`
3. Type `git commit -m "modified README.md"`
4. Type `git log`
5. Type `git push`
...
6. Navigate to your GitLab.com project in your browser and confirm that your local changes were pushed up.
...
7. Click the **Commit** button to finalize the changes on the remote copy of **temporary_branch**. Since you made this change on the server, the server repository is now one commit ahead of your local repository.
