...
If you have engaged a phisher by replying to an email, clicking on a link, have sent and received text messages, or have purchased goods requested by the phisher, please [engage the Security Engineer on-call](/handbook/security/security-operations/sirt/engaging-security-on-call.html#phishing).
Further information on GitLab's security response program is described in our [Incident Response](/handbook/security/security-operations/sirt/sec-incident-response.html) guide.
{: .alert .alert-danger}
...
* Please forward the email via the ![PhishAlarm button](/handbook/security/security-assurance/images/PhishAlarm.png) or as an attachment to phishing@gitlab.com using these [instructions](#what-to-do-if-you-suspect-an-email-is-a-phishing-attack). Knowing this is a phishing simulation, please avoid discussing with anyone else or feel compelled to post a screenshot of the email received in Slack as it may skew the results of the phishing exercise.
...
**Note:** Forwarding phishing emails to `phishing@gitlab.com` requires additional steps by following the [manual submission instructions](#manual-submission-of-phishing-email-to-phishinggitlabcom) below.
{: .note}
...
To submit an email via PhishAlarm to GitLab's Security Team using Gmail:
  ...
  2. Follow the instructions to report the Phish
  3. Confirm you are ready to report the Phishing email
  4. Receive a confirmation that the email has been forwarded to GitLab Security Team for further investigation.
...
[use the `/security` Slack command](https://about.gitlab.com/handbook/security/security-operations/sirt/engaging-security-on-call.html#engage-the-security-engineer-on-call).
