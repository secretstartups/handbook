...
flowchart TD
    ...
    QuickFix --> |No| HotPatch(Deploy a Hot Patch with a &quothacky&quot mitigation while the long term<br>fix is being figured out by the team that owns the feature.<br>This blocks all other deployments and we want to avoid this at all costs.)
    ...
    PublicFix -->|We really can't risk making the fix in public,<br>even with a &quotlow profile&quot MR| CriticalRelease(Critical Security Release)
