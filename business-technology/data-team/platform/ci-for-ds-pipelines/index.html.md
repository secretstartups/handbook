...
- Finally, let's look at the [config.yml](https://gitlab.com/gitlab-data/data-science-ci-example/-/blob/main/config.yml). Here we can configure certain variables for training our model:
  ...
  - `optuna` configurations: The example notebook runs an xgboost model with [Optuna](https://optuna.org/). There are _a lot_ of customizations that are possible with this setup, but to keep it simple, we have only included:
