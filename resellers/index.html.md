...
3. **GitLab Basics** - GitLab Basics is the foundational technical training for post-sales technical professionals and can also be a valuable training for pre-sales technical professionals.  This curriculum is still in pilot for the first half of 2020.  Partners will be able to participate by invitation only during the pilot period.  As we continue to develop our services enablement offerings, partners will need to develop a custom plan with their Channel Account Managers to meet the certification requirements.
 ...
 On-demand, online training courses are available on the GitLab Partner Portal.  Registration for courses delivered to a live audience will also be available on the Partner Portal. Sales Core training takes aproximately 3.5 hours to complete, Solution Architect training takes aproximately 5 hours to complete. 
...
*   **Services Attach Discount and Rebate** is an incentive for partner-delivered services provided to the end user related to their use of the GitLab Software (Core/Free and Starter/Bronze sales do not qualify). Services include product-related services (such as deployment, integration and migration), operational and strategic. For resale partners, the first Services Attach incentive is administered as an upfront 2.5% discount from the GitLab license price on the initial sale. Partners that do not resell or resellers that sell their own services after the initial sale may earn a rebate of 2.5% of the net license price of the most recent software sale for a period of up to six (6) months.
    ...
    *   Partner Services engagements must meet the following partner services deal size minimums:
    ...
    Services Attach requests shall be submitted to GitLab via the Partner Portal as a deal registration, must include reasonable information and documentation (as requested by GitLab) to demonstrate the Referral Clients receipt of the Partner Professional Services and must be approved by GitLab Sales.
...
* **To receive MDF, you must:**
  *  Submit a proposal to GitLab
  *  Have an authorized GitLab Marketing Manager review and approve the proposal
  *  Promptly submit the required proof of performance documentation to GitLab following the activity
...
All orders require an executed[ EULA](https://about.gitlab.com/terms/print/gitlab_subscription_terms.pdf) when there's a new subscription or an add-on. There are 2 methods of obtaining a EULA:
...
*   MDF Request – All MDF requests should be submitted in advance of each planned marketing activity, using the MDF request form found on the GitLab Partner Portal. Note: MDF requests must be submitted prior to the start of an activity, or the request may be rejected.
...
#####  Proof of Performance (POP) and Claiming Funds
 ...
 The GitLab MDF program is reimbursement-based. For prior approved marketing claims to be reimbursed, the following conditions must be met:


*   You must submit a payment claim within 30 calendar days of completion of the activity via the Partner Portal.
...
Access to MDF fund requests will be granted only to select individuals at a partner.  From the GitLab Partner Portal home page, navigate to Marketing and select “MDF Request.” The “MDF Request” option only appears if you are authorized to participate. Upon selecting MDF Request, complete the first part of the firm to create a fund request. Once completed, submit it and it will be routed to a GitLab Representative for approval.
