...
* What is the conversion rate (INQ>MQL,MQL>SAO,SAO>WON) for our tactics (i.e. webcasts, gated content,etc.)? How much pipeline are our different tactics generating?
...
* How is the performance trending MoM, QoQ, YoY based on the main KPIs?
* How does the velocity (INQ>MQL, MQL>SAO, SAO>WON) differ by campaign group?
...
* How many MQLs did the campaign generate by Persona (Executives, Managers, Practitioners, Unclassified)? Note: the persona classification is mirroring the setup in Marketo.
* Which accounts engage with our integrated campaigns? Are these accounts on TAL (Target Account List)?
