...
Open merge requests sometimes become idle (not being updated by a human in more than a month).  Engineering managers will receive an idle MR triage issue once a month that lists all (non-WIP) for their group that are idle so that they can analyze them to determine if action should be taken (such as nudging the author/reviewer/maintainer).  This assists in getting merge requests merged in a reasonable amount of time (which we track as the metric MTTR: Mean Time to Merge).

Open merge requests may also have other properties that indicate that the engineering manager may want to research them in order to potentially take action in order to improve effeciency.  These properties include the number of threads (which when very high may indicate that the plan for the MR should be revisited or that a syncrhonous discussion may be advisable) and number of pipelines (as a high number of pipelines may indicate that the plan for the MR should be revisited).  These metrics are not yet used to include an MR in a triage issue, however they are available in a [SiSense dashboard](https://app.periscopedata.com/app/gitlab/663398/Merge-requests-that-may-warrant-attention).  Engineering managers are encouraged to check this dashboard for their group periodically (once or twice a month) in the interim.
